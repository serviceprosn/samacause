  ...
  if (!currentUser || currentUser.role !== 'admin') {
    return (
      <div className="animate-fade-in" style={{ padding: '4rem 2rem', textAlign: 'center' }}>
        ...
        <h2 style={{ fontSize: '1.5rem', fontWeight: 800, marginTop: '1rem' }}>Accès Administrateur Restreint</h2>
  ...
  return (
    <div className="animate-fade-in" style={{ paddingBottom: '3rem' }}>
      ...
      <section style={{ marginBottom: '3rem' }}>
        ...
        <div className="grid-cols-4">
          ...
          <div className="premium-card">
            <span style={{ fontSize: '1.5rem' }}>👤</span>
            <div style={{ marginTop: '0.5rem' }}>
              <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary-light)' }}>Membres Enregistrés</span>
              <h3 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--primary)' }}>
                {kpis.totalUsers}
              </h3>
            </div>
      ...
      <section style={{ marginBottom: '3rem' }}>
        ...
        {totalPending === 0 ? (
          <div style={{ textAlign: 'center', padding: '3rem', background: 'var(--light-card)', border: '1px solid var(--border-light)', borderRadius: 'var(--radius-md)' }}>
            <p style={{ fontWeight: 600 }}>Toutes les campagnes ont été modérées ! 🎉</p>
                ...
                >
                  ...
                  {isExpanded && (
                    <div className="animate-fade-in" style={{ borderTop: '1px solid var(--border-light)', paddingTop: '1.25rem', marginTop: '0.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                      ...
                      <div>
                        <h4 style={{ fontSize: '0.9rem', fontWeight: 800, marginBottom: '0.5rem' }}>📝 Description & Lettre de Pétition :</h4>
                      ...
                      {rejectingCampaignId === p.id && (
                        <div className="animate-slide-up" style={{ background: 'rgba(239,68,68,0.05)', padding: '1rem', borderRadius: 'var(--radius-sm)', border: '1px dashed var(--danger)' }}>
                          ...
                          <textarea
                            rows={3}
                            placeholder="Veuillez expliquer à l'utilisateur ce qui bloque son dossier (ex: reformulation de la lettre, problème de destinataire, etc.)..."
                            className="premium-card"
                ...
                >
                  ...
                  {isExpanded && (
                    <div className="animate-fade-in" style={{ borderTop: '1px solid var(--border-light)', paddingTop: '1.25rem', marginTop: '0.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                      ...
                      {/* Image Gallery */}
                      <div>
                        <h4 style={{ fontSize: '0.9rem', fontWeight: 800, marginBottom: '0.5rem' }}>🖼️ Galerie Photos complémentaires (Max 5 photos) :</h4>
                        {(!c.gallery || c.gallery.length === 0) ? (
                          <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary-light)', fontStyle: 'italic' }}>Aucune photo de terrain ajoutée à la galerie.</p>
                        ) : (
                      ...
                      {rejectingCampaignId === c.id && (
                        <div className="animate-slide-up" style={{ background: 'rgba(239,68,68,0.05)', padding: '1rem', borderRadius: 'var(--radius-sm)', border: '1px dashed var(--danger)' }}>
                          ...
                          <textarea
                            rows={3}
                            placeholder="Veuillez préciser à l'utilisateur quel document justificatif ou quelle photo manque à l'appel pour pouvoir activer la cagnotte..."
                            className="premium-card"
                            style={{ width: '100%', padding: '0.6rem', fontSize: '0.85rem', background: 'white', resize: 'none', marginBottom: '0.75rem' }}
                            value={rejectionFeedback}
                            onChange={(e) => setRejectionFeedback(e.target.value)}
      ...
      <section style={{ marginTop: '2rem' }}>
        <h2 style={{ fontSize: '1.4rem', fontWeight: 800, marginBottom: '1.25rem' }}>👤 Annuaire des Citoyens, KYC & Modération des Comptes</h2>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {usersList.map((user) => {
            ...
            const getStatusBadge = (status?: string) => {
              switch (status) {
                case 'verified':
                  return <span style={{ padding: '0.2rem 0.5rem', fontSize: '0.7rem', borderRadius: '4px', background: 'rgba(0,133,63,0.1)', color: 'var(--primary)', fontWeight: 'bold' }}>✓ Vérifié</span>;
                case 'pending':
                ...
                case 'rejected':
                  return <span style={{ padding: '0.2rem 0.5rem', fontSize: '0.7rem', borderRadius: '4px', background: 'rgba(239,68,68,0.1)', color: 'var(--danger)', fontWeight: 'bold' }}>✕ Rejeté</span>;
                default:
                ...
                {isUserExpanded && (
                  ...
                  >
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                      ...
                      <div style={{ background: 'var(--light)', padding: '0.75rem', borderRadius: '4px' }}>
                        <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary-light)', display: 'block' }}>Téléphone</span>
                    ...
                    <div>
                      <h5 style={{ fontWeight: 800, fontSize: '0.85rem', marginBottom: '0.5rem' }}>📁 Pièces Justificatives Téléversées</h5>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
                        ...
                        {/* Selfie */}
                        <div style={{ border: '1px solid var(--border-light)', borderRadius: '6px', overflow: 'hidden', background: '#fafafa', textAlign: 'center' }}>
                          <span style={{ display: 'block', fontSize: '0.65rem', padding: '0.35rem', background: '#eee', fontWeight: 600 }}>Selfie de contrôle</span>
                          {user.selfie ? (
                            <img 
                              src={user.selfie} 
                              alt="Selfie" 
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1.5rem', borderTop: '1px solid var(--border-light)', paddingTop: '1.25rem' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-secondary-light)' }}>Modération de l'identité</span>
                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                          <button
                            type="button"
                            className="btn btn-primary"
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                        <label htmlFor={`role-select-${user.id}`} style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-secondary-light)' }}>Rôle plateforme</label>
                        <select
